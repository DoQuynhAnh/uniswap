...
export function isDevEnv() {
    if (isInterface) {
        ...
        if (!chrome.runtime) {
            ...
            console.warn('Avoid using `isDevEnv()` inside the injected script. Use `__DEV__` instead. ' +
...
export function isBetaEnv() {
    if (isInterface) {
        ...
        if (!chrome.runtime) {
            // eslint-disable-next-line no-console
            console.warn('Avoid using `isBetaEnv()` inside the injected script. ' +
                '`chrome.runtime` is only available when the injected script is running inside a trusted site (`app.uniswap.org`). ' +
...
export function isProdEnv() {
    if (isInterface) {
        ...
        if (!chrome.runtime) {
            // eslint-disable-next-line no-console
            console.warn('Avoid using `isProdEnv()` inside the injected script. ' +
                '`chrome.runtime` is only available when the injected script is running inside a trusted site (`app.uniswap.org`). ' +
