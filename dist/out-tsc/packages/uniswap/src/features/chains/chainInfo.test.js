...
import { getQuicknodeChainId, getQuicknodeChainIdPathSuffix, getQuicknodeEndpointUrl, } from 'uniswap/src/features/chains/chainInfo';
...
describe('getQuicknodeEndpointUrl', () => {
    ...
    it('throws error for unsupported chain', () => {
        ...
        expect(() => getQuicknodeEndpointUrl(999999)).toThrow('Chain 999999 does not have a corresponding QuickNode chain ID');
    });
    it('handles all supported chains without throwing', () => {
        ...
        supportedChains.forEach((chainId) => {
            ...
            expect(url).toEqual(`https://test-endpoint${chainId === UniverseChainId.Mainnet ? '' : `.${getQuicknodeChainId(chainId)}`}.quiknode.pro/test-token-123${getQuicknodeChainIdPathSuffix(chainId)}`);
        });
    });
});
...
//# sourceMappingURL=chainInfo.test.js.map