...
describe(useSearchTokensGql, () => {
    ...
    it('searches all networks for address search with no chain filter', async () => {
        ...
        await waitFor(async () => {
            ...
            expect((_a = result.current.data) === null || _a === void 0 ? void 0 : _a.every((item) => item.isFromOtherNetwork === false)).toBe(true);
        });
    });
    it('separates tokens by network for address search with chain filter', async () => {
        ...
        const { result } = renderHook(() => useSearchTokensGql('0x1234567890123456789012345678901234567890', UniverseChainId.Mainnet, false), {
            resolvers,
        });
        await waitFor(async () => {
            expect(result.current.loading).toEqual(false);
            ...
            if (result.current.data) {
                ...
                const ethereumResults = result.current.data.filter((item) => item.currency.chainId === Number(UniverseChainId.Mainnet));
                ...
                const otherNetworkResults = result.current.data.filter((item) => item.currency.chainId !== Number(UniverseChainId.Mainnet));
...
//# sourceMappingURL=searchTokensGql.test.js.map