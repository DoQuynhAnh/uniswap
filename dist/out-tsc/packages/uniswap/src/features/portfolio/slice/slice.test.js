...
import { addTokensToBalanceOverride, initialPortfolioState, portfolioReducer, removeExpiredBalanceOverrides, removeTokenFromBalanceOverride, } from 'uniswap/src/features/portfolio/slice/slice';
...
describe(portfolioReducer, () => {
    describe(addTokensToBalanceOverride, () => {
        it('adds token overrides for a valid address', () => {
            ...
            const state = portfolioReducer(initialPortfolioState, addTokensToBalanceOverride({
                ownerAddress: TEST_ADDRESS1,
                currencyIds: [currencyId],
            }));
            expect(state.tokenBalanceOverrides[TEST_ADDRESS1]).toBeDefined();
            expect((_a = state.tokenBalanceOverrides[TEST_ADDRESS1]) === null || _a === void 0 ? void 0 : _a[currencyId]).toBeDefined();
            expect((_c = (_b = state.tokenBalanceOverrides[TEST_ADDRESS1]) === null || _b === void 0 ? void 0 : _b[currencyId]) === null || _c === void 0 ? void 0 : _c.updatedAt).toBeGreaterThan(0);
        });
        ...
        it('updates existing overrides', () => {
            const currencyId = buildCurrencyId(TEST_CHAIN_ID, TEST_TOKEN_ADDRESS);
            let state = portfolioReducer(initialPortfolioState, addTokensToBalanceOverride({
                ownerAddress: TEST_ADDRESS1,
                currencyIds: [currencyId],
            }));
            ...
            state = portfolioReducer(state, addTokensToBalanceOverride({
                ownerAddress: TEST_ADDRESS1,
                currencyIds: [currencyId],
            }));
    ...
    describe(removeTokenFromBalanceOverride, () => {
        it('removes token override for valid address', () => {
            ...
            expect(state.tokenBalanceOverrides[TEST_ADDRESS1]).toBeDefined();
            state = portfolioReducer(state, removeTokenFromBalanceOverride({
                ownerAddress: TEST_ADDRESS1,
        ...
        it('ignores invalid addresses', () => {
            ...
            const state = portfolioReducer(initialState, removeTokenFromBalanceOverride({
                ownerAddress: 'invalid-address',
                chainId: TEST_CHAIN_ID,
                tokenAddress: TEST_TOKEN_ADDRESS,
            }));
