...
import { createCreatePositionAsyncStep, createIncreasePositionAsyncStep, createIncreasePositionStep, } from 'uniswap/src/features/transactions/liquidity/steps/increasePosition';
import { createMigratePositionAsyncStep, createMigratePositionStep, } from 'uniswap/src/features/transactions/liquidity/steps/migrate';
...
export function generateLPTransactionSteps(txContext) {
    ...
    if (isValidLP) {
        ...
        const { action, approveToken0Request, approveToken1Request, approvePositionTokenRequest, token0PermitTransaction, token1PermitTransaction, positionTokenPermitTransaction, } = txContext;
        const revokeToken0 = createRevocationTransactionStep(txContext.revokeToken0Request, action.currency0Amount.currency.wrapped);
        const revokeToken1 = createRevocationTransactionStep(txContext.revokeToken1Request, action.currency1Amount.currency.wrapped);
        ...
        const positionTokenPermitTransactionStep = createPermit2TransactionStep(positionTokenPermitTransaction, action.currency1Amount, [action.currency0Amount.currency, action.currency1Amount.currency]);
            ...
            case 'migrate':
                if (txContext.unsigned) {
                    return orderMigrateLiquiditySteps({
                        permit: createPermit2SignatureStep(txContext.permit.typedData, new Token(1, ADDRESS_ZERO, 1, 'Uniswap V3 Positions NFT-V1', 'Uniswap V3 Positions NFT-V1')),
                        migrate: createMigratePositionAsyncStep(txContext.migratePositionRequestArgs, txContext.permit.typedData.values.deadline),
...
//# sourceMappingURL=generateLPTransactionSteps.js.map