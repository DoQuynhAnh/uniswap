...
describe('formatPriceImpact', () => {
    ...
    const mockFormatPercent = jest.fn((value) => {
        if (value === undefined || value === null) {
            return '-mocked%';
        }
        return `${value}-mocked%`;
    });
    const createPercent = (numerator, denominator = 100) => {
    ...
    it('should format a positive price impact (negative effect for user)', () => {
        ...
        expect(result === null || result === void 0 ? void 0 : result.startsWith('+')).toBe(false);
    });
...
//# sourceMappingURL=formatPriceImpact.test.js.map