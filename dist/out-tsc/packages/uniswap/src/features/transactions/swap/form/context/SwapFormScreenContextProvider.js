...
import { SwapFormScreenContext, } from 'uniswap/src/features/transactions/swap/form/context/SwapFormScreenContext';
...
export function SwapFormScreenContextProvider({ tokenColor, children, }) {
    const { walletNeedsRestore, screen } = useTransactionModalContext();
    const { derivedSwapInfo, exactAmountFiat, exactAmountToken, exactCurrencyField, focusOnCurrencyField, selectingCurrencyField, input, isFiatMode, output, hideFooter, } = useSwapFormContext();
    ...
    const { outputTokenHasBuyTax, exactOutputWillFail, exactOutputWouldFailIfCurrenciesSwitched } = useMemo(() => getExactOutputWillFail({ currencies }), [currencies]);
    ...
    const isBridge = Boolean(input && output && (input === null || input === void 0 ? void 0 : input.chainId) !== (output === null || output === void 0 ? void 0 : output.chainId));
    ...
    return _jsx(SwapFormScreenContext.Provider, { value: contextValue, children: children });
}
//# sourceMappingURL=SwapFormScreenContextProvider.js.map