...
describe('processSectionsToRows', () => {
    const mockToken = new Token(1, // chainId
    ...
    const mockTokenOption = {
        type: OnchainItemListOptionType.Token,
        currencyInfo: mockCurrencyInfo,
        quantity: null,
        balanceUSD: null,
    };
    const createMockTokenSection = (sectionKey, data = [mockTokenOption], name) => ({
        sectionKey,
        data,
        name,
    });
    // Type guard functions
    const isHeaderRow = (row) => row.type === ProcessedRowType.Header;
    const isItemRow = (row) => row.type === ProcessedRowType.Item;
    ...
    it('preserves section metadata in processed items', () => {
        const rightElement = _jsx(_Fragment, { children: "Right" });
        const endElement = _jsx(_Fragment, { children: "End" });
        const section = {
...
//# sourceMappingURL=processSectionsToRows.test.js.map