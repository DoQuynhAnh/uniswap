 ...
 */
export function ContextMenu({ children, menuItems, isPlacementAbove = false, isPlacementRight = false, offsetX = 0, offsetY = 0, onPressAny, triggerMode, disabled = false, isOpen, closeMenu, openMenu, }) {
    ...
    const recalculateMenuPosition = useCallback(() => {
        if (isOpen && triggerRef.current) {
            triggerRef.current.measure((_fx, _fy, triggerWidth, triggerHeight, triggerX, triggerY) => {
                ...
                const estimatedMenuHeight = menuItems.length * MENU_OPTION_HEIGHT + MENU_OPTION_GAP * (menuItems.length - 1) + MENU_PADDING * 2;
                const getTop = () => {
                    const aboveTriggerY = triggerY - estimatedMenuHeight - offsetY;
                    const belowTriggerY = triggerY + triggerHeight + offsetY;
    ...
    const menuSheetItems = useMemo(() => {
        return menuItems === null || menuItems === void 0 ? void 0 : menuItems.map(({ label, textColor, Icon, iconColor, disabled: itemDisabled, onPress: onPressAction, showDivider, closeDelay }, index) => (_jsxs(Fragment, { children: [showDivider && _jsx(Separator, { my: "$spacing6" }), _jsx(DropdownMenuSheetItem, { variant: "medium", label: label, textColor: textColor, icon: Icon && _jsx(Icon, { size: "$icon.24", color: iconColor }), height: MENU_OPTION_HEIGHT, disabled: itemDisabled, closeDelay: (closeDelay !== null && closeDelay !== void 0 ? closeDelay : 0) + ANIMATION_TIME, handleCloseMenu: () => {
                        closeMenu();
                        setIsMenuVisible(false);
                    }, onPress: () => {
                        try {
                            ...
                            onPressAny === null || onPressAny === void 0 ? void 0 : onPressAny({ name: label, index, indexPath: [index] });
                        }
                        catch (error) {
                            logger.error(error, {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(Portal, { display: isOpen || isMenuVisible ? 'flex' : 'none', contain: "none", position: "unset", 
                // pass events through if menu is fading out
                pointerEvents: !isOpen ? 'none' : 'auto', onPress: (e) => {
                    e.stopPropagation();
                }, children: _jsx(Flex, { height: "100%", width: "100%", top: 0, left: 0, backgroundColor: "transparent", zIndex: zIndexes.overlay, onPress: handleMenuClose, children: _jsx(AnimatePresence, { children: isMenuVisible && (_jsx(Flex, { justifyContent: "flex-start", alignItems: "flex-start", backgroundColor: "$transparent", top: position.top, left: position.left, position: "absolute", animation: "200ms", enterStyle: {
                                opacity: 0,
                                y: isAboveTrigger ? ANIMATION_START_POINT : -ANIMATION_START_POINT,
                            }, exitStyle: {
                                opacity: 0,
                                y: isAboveTrigger ? ANIMATION_START_POINT : -ANIMATION_START_POINT,
                            }, children: _jsx(Flex, { backgroundColor: "$surface1", p: MENU_PADDING, borderRadius: "$rounded20", borderColor: "$surface3", borderWidth: "$spacing1", gap: MENU_OPTION_GAP, alignItems: "flex-start", width: CONTEXT_MENU_WIDTH, shadowRadius: "$spacing4", shadowColor: "$shadowColor", children: menuSheetItems }) })) }) }) }), _jsx(Flex, { children: openMenu ? (_jsx(TouchableArea, { disabled: disabled, onPress: isLongPress ? undefined : openMenu, onLongPress: isLongPress ? openMenu : undefined, children: _jsx(Flex, { ref: triggerRef, onLayout: recalculateMenuPosition, children: children }) })) : (
                // if openMenu is undefined, {children} controls menu open/close state. Don't want to interfere with nested TouchableAreas
                _jsx(Flex, { ref: triggerRef, onLayout: recalculateMenuPosition, children: children })) })] }));
}
//# sourceMappingURL=ContextMenuV2.native.js.map