...
import { createEmptyBalanceOption, useCurrencyInfosToTokenOptions, } from 'uniswap/src/components/TokenSelector/hooks/useCurrencyInfosToTokenOptions';
...
describe(usePortfolioTokenOptions, () => {
    ...
    describe('shown tokens', () => {
        ...
        // Portfolio balances
        const ethPortfolioBalanceTokenOption = {
            ...portfolioBalance({ fromBalance: ethTokenBalance }),
            type: OnchainItemListOptionType.Token,
        };
        const usdcPortfolioBalanceTokenOption = {
...
describe(useTrendingTokensOptions, () => {
    beforeEach(() => {
        ...
        mockTokenRankingsStatToCurrencyInfo.mockImplementation((tokenRankingsStat) => {
            var _a, _b;
            return ({
                currencyId: buildCurrencyId((_a = fromGraphQLChain(tokenRankingsStat.chain)) !== null && _a !== void 0 ? _a : UniverseChainId.Mainnet, tokenRankingsStat.address),
                currency: {
                    address: tokenRankingsStat.address,
                    chainId: (_b = fromGraphQLChain(tokenRankingsStat.chain)) !== null && _b !== void 0 ? _b : UniverseChainId.Mainnet,
    ...
    it('returns trending token options when there is data', async () => {
        ...
        await waitFor(() => {
            expect(result.current).toEqual({
                loading: false,
                data: expect.toIncludeSameMembers(tokenBalances.map((t) => ({
...
describe(useFavoriteTokensOptions, () => {
    ...
    const cases = [
        ...
        },
        {
            ...
            output: {
                data: expect.toIncludeSameMembers(favoriteTokenBalances.map((balance) => {
