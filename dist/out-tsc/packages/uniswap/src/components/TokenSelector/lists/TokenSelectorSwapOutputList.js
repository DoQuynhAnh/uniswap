...
function useTokenSectionsForSwapOutput({ activeAccountAddress, chainFilter, oppositeSelectedToken: input, }) {
    var _a;
    ...
    // if there is no chain filter, first check if the input token has a chainId, fallback to defaultChainId
     } = useCommonTokensOptionsWithFallback(activeAccountAddress, (_a = chainFilter !== null && chainFilter !== void 0 ? chainFilter : input === null || input === void 0 ? void 0 : input.chainId) !== null && _a !== void 0 ? _a : defaultChainId);
    const { data: bridgingTokenOptions, error: bridgingTokenOptionsError, refetch: refetchBridgingTokenOptions, loading: bridgingTokenOptionsLoading, shouldNest: shouldNestBridgingTokens, } = useBridgingTokensOptions({ oppositeSelectedToken: input, walletAddress: activeAccountAddress, chainFilter });
...
function _TokenSelectorSwapOutputList({ onSelectCurrency, activeAccountAddress, chainFilter, isKeyboardOpen, oppositeSelectedToken: input, }) {
