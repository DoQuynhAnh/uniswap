...
import { NATIVE_ANALYTICS_ADDRESS_VALUE, areCurrencyIdsEqual, buildCurrencyId, buildNativeCurrencyId, currencyAddress, currencyId, currencyIdToAddress, currencyIdToChain, currencyIdToGraphQLAddress, getCurrencyAddressForAnalytics, isCurrencyIdValid, isNativeCurrencyAddress, } from 'uniswap/src/utils/currencyId';
...
describe('currencyId', () => {
  ...
  `('currencyId=$currencyId should return expected=$expected', ({ _currencyId, expected }) => {
    ...
    it.each([[UniverseChainId.Mainnet, DAI.address, `${UniverseChainId.Mainnet}-${DAI.address}`]])('buildCurrencyId builds correct ID for chainId=%s + address=%s = %s', (chainId, address, expectedId) => {
    ...
    ])('areCurrencyIdsEqual returns correct comparison for currencyId1=%s and currencyId2=%s = %s', (currencyId1, currencyId2, expected) => {
    ...
    ])('isNativeCurrencyAddress returns correct result for chainId=%s + address=%s = %s', (chainId, address, expected) => {
