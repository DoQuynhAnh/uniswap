...
import { DatadogIgnoredErrorsConfigKey, DatadogSessionSampleRateKey, DynamicConfigs, } from 'uniswap/src/features/gating/configs';
...
function beforeSend(event, context) {
    ...
    if (event.error && event.type === 'error') {
        ...
        const ignoredErrors = getDynamicConfigValue(DynamicConfigs.DatadogIgnoredErrors, DatadogIgnoredErrorsConfigKey.Errors, []);
        const ignoredError = ignoredErrors.find(({ messageContains }) => { var _a; return (_a = event.error) === null || _a === void 0 ? void 0 : _a.message.includes(messageContains); });
        if (ignoredError && Math.random() > ignoredError.sampleRate) {
            return false;
        }
        Object.defineProperty(event.error, 'stack', {
            value: (_a = event.error.stack) === null || _a === void 0 ? void 0 : _a.replace(/chrome-extension:\/\/[a-z]{32}/gi, ''),
            writable: false,
            configurable: true,
        });
    }
    if (event.type === 'resource' && event.resource.url.includes('graphql')) {
        const requestBody = (_b = context.requestInit) === null || _b === void 0 ? void 0 : _b.body;
        if (requestBody) {
...
export async function initializeDatadog(appName) {
    if (!datadogEnabledBuild) {
        return;
    }
    const sessionSampleRate = getDynamicConfigValue(DynamicConfigs.DatadogSessionSampleRate, DatadogSessionSampleRateKey.Rate, isExtension ? EXTENSION_DEFAULT_DATADOG_SESSION_SAMPLE_RATE : INTERFACE_DEFAULT_DATADOG_SESSION_SAMPLE_RATE);
    ...
    const sessionIsSampled = (internalContext === null || internalContext === void 0 ? void 0 : internalContext.session_id) !== undefined;
    ...
    for (const [_, flagKey] of [...WEB_FEATURE_FLAG_NAMES.entries(), ...WALLET_FEATURE_FLAG_NAMES.entries()]) {
        ...
        flagKey.replaceAll('-', '_'), getStatsigClient().checkGate(flagKey));
    }
    for (const experiment of Object.values(Experiments)) {
        datadogRum.addFeatureFlagEvaluation(
        // Datadog has a limited set of accepted symbols in feature flags
        // https://docs.datadoghq.com/real_user_monitoring/guide/setup-feature-flag-data-collection/?tab=reactnative#feature-flag-naming
        `experiment_${experiment.replaceAll('-', '_')}`, getStatsigClient().getExperiment(experiment).groupName);
    }
}
