 ...
 */
export function SegmentedControl({ options, selectedOption, onSelectOption, size = 'default', disabled, fullWidth, outlined = true, gap, }) {
    ...
    return (<Tabs unstyled activationMode="manual" orientation="horizontal" value={selectedOption} onValueChange={(option) => {
      ...
      <OptionsSelector disablePassBorderRadius unstyled backgroundColor="transparent" fullWidth={fullWidth} outlined={outlined} loop={false} size={size} gap={gap}>
        ...
        <AnimatePresence>
          {activeAt && (<TabsRovingIndicator height={activeAt.height} width={activeAt.width} x={activeAt.x - TOGGLE_PADDING + activeIndicatorXAdjustment} y={activeAt.y - TOGGLE_PADDING + activeIndicatorYAdjustment - (isWeb && !outlined ? 1 : 0)}/>)}
        </AnimatePresence>
      </OptionsSelector>
    </Tabs>);
}
