...
import { DEFAULT_MIN_HEIGHT, DEFAULT_MIN_WIDTH, useAutoDimensions, } from 'ui/src/components/touchable/TouchableArea/useAutoDimensions';
// We're testing the mobile version of this hook
jest.mock('utilities/src/platform', () => ({
    isMobileApp: true,
}));
// Helper to create a mock LayoutChangeEvent
const createLayoutChangeEvent = (width, height) => {
    return {
        nativeEvent: {
...
describe('useAutoDimensions', () => {
    ...
    it('should return dimensions from params when shouldConsiderMinimumDimensions is false', () => {
        ...
        const { result } = renderHook(() => useAutoDimensions({ width: widthParam, height: heightParam, shouldConsiderMinimumDimensions: false }));
    ...
    it('should call the provided onLayout callback', () => {
        ...
        const { result } = renderHook(() => useAutoDimensions({ onLayout: mockOnLayout, shouldConsiderMinimumDimensions: true }));
    ...
    it('should prioritize internal dimensions over params when shouldConsiderMinimumDimensions is true and dimensions are set', () => {
        const widthParam = 100;
        const heightParam = 200;
        const { result } = renderHook(() => useAutoDimensions({ width: widthParam, height: heightParam, shouldConsiderMinimumDimensions: true }));
    ...
    it('should handle layout event updates correctly', () => {
        const { result, rerender } = renderHook(({ shouldConsiderMinimumDimensions }) => useAutoDimensions({ shouldConsiderMinimumDimensions }), { initialProps: { shouldConsiderMinimumDimensions: true } });
...
//# sourceMappingURL=useAutoDimensions.test.js.map