...
const ButtonComponent = forwardRef(function Button({ children, icon, fill = true, shouldAnimateBetweenLoadingStates = true, variant = 'default', focusScaling = 'default', emphasis = 'primary', size = 'medium', loading, iconPosition: propIconPosition = 'before', isDisabled: propDisabled, onPress, ...props }, ref) {
    var _a;
    ...
    return (<CustomButtonFrame ref={ref} fill={fill} focusScaling={focusScaling} emphasis={emphasis} variant={variant} size={size} iconPosition={iconPosition} isDisabled={isDisabled} 
    // TODO(WEB-6347): Re-enable disabled prop once tamagui Adapt issue is fixed
    // disabled={props.onDisabledPress ? false : isDisabled}
    custom-background-color={customBackgroundColor} dd-action-name={(_a = props['dd-action-name']) !== null && _a !== void 0 ? _a : (typeof children === 'string' ? children : undefined)} {...props} onPress={handleOnPress}>
      <ThemedIcon custom-background-color={customBackgroundColor} isDisabled={isDisabled} emphasis={emphasis} size={size} variant={variant} typeOfButton="button">
      ...
      {loading ? (<ThemedSpinningLoader isDisabled={isDisabled} emphasis={emphasis} size={size} variant={variant} typeOfButton="button"/>) : null}
