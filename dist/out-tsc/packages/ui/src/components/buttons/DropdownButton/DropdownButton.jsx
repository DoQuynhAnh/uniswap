...
const LeftContainer = ({ elementPositioning, children, icon, label }) => {
    if (elementPositioning === 'grouped' && icon && label) {
        return (<Flex row alignItems="center" gap="$gap12">
        {children}
      </Flex>);
    }
    return <Fragment>{children}</Fragment>;
};
const DropdownButtonComponent = forwardRef(function DropdownButton({ children, emphasis = 'secondary', icon, isDisabled, elementPositioning = 'equal', isExpanded, ...props }, ref) {
    ...
    const iconGroupItemHover = useMemo(() => isExpanded
        ? {
            color: EXPANDED_HOVER_COLOR,
        }
        : undefined, [isExpanded]);
    const iconColor = isExpanded ? EXPANDED_COLOR : undefined;
    return (<DropdownButtonFrame ref={ref} iconPosition={getIconPosition('before')} emphasis={emphasis} isDisabled={disabled} isExpanded={isExpanded} {...props}>
      ...
      <ThemedIcon isDisabled={isDisabled} emphasis={emphasis} size={props.size} variant="default" typeOfButton="button">
        <RotatableChevron color={iconColor} animation="fast" direction={isExpanded ? 'up' : 'down'} $group-item-hover={iconGroupItemHover}/>
      </ThemedIcon>
    </DropdownButtonFrame>);
});
/**
