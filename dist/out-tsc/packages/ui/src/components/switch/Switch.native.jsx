...
import { SWITCH_THUMB_HEIGHT, SWITCH_THUMB_PADDING, SWITCH_TRACK_HEIGHT, SWITCH_TRACK_WIDTH, } from 'ui/src/components/switch/shared';
...
export const Switch = memo(function Switch({ checked, defaultChecked = false, onCheckedChange, disabled, variant = 'default', style, testID, pointerEvents, }) {
    const colors = useSporeColors();
    const isBranded = variant === 'branded';
    const progress = useSharedValue((checked !== null && checked !== void 0 ? checked : defaultChecked) ? 1 : 0);
    useEffect(() => {
    ...
    const thumbStyle = useAnimatedStyle(() => {
        ...
        return {
            transform: [
                {
                    translateX: withTiming(interpolate(isOn, [0, 1], [0, SWITCH_TRACK_WIDTH - SWITCH_THUMB_HEIGHT - SWITCH_THUMB_PADDING * 2]), ANIMATION_CONFIG),
                },
            ],
    ...
    const containerStyle = useMemo(() => (pointerEvents === 'none' ? { pointerEvents: 'none' } : {}), [pointerEvents]);
    return (<Pressable disabled={disabled} hitSlop={12} style={containerStyle} testID={testID} onPress={handlePress}>
...
function getTrackColor(isOn, disabled, isBranded, colors) {
    'worklet';
    if (disabled && !isOn) {
...
function getThumbColor(isOn, disabled, isBranded, colors) {
    'worklet';
    if (disabled && !isOn) {
...
function getIconColor(isOn, disabled, isBranded, colors) {
    'worklet';
    if (disabled && !isOn) {
        return colors.white.val;
    }
...
//# sourceMappingURL=Switch.native.jsx.map