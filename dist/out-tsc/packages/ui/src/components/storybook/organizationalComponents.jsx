...
const PageTitle = ({ description, figmaUrl, componentName, componentUrl, }) => (<>
...
const VariantHeaderContainer = ({ children }) => {
    return (<Flex px="$padding8" py="$padding6" backgroundColor="$surface3" borderRadius="$rounded12">
      {children}
    </Flex>);
};
const EmphasizedText = ({ children }) => (<Text variant="body4" fontWeight="bold" fontStyle="italic" color="$statusCritical">
    {children}
  </Text>);
/**
 ...
 */
export const EmphasizableText = ({ children }) => {
    // Split on curly braces, keeping the delimiters
    const parts = children.split(/(\{[^}]+\})/g);
    return (<Text variant="body3">
      {parts.map((part, idx) => {
...
//# sourceMappingURL=organizationalComponents.jsx.map