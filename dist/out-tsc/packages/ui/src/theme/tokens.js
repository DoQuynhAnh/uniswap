...
export const getIsTokenFormat = (value) => {
    return value[0] === '$';
};
// Exported for testing
export const getIsValidSporeColor = (value) => {
...
export const validateColorValue = (value) => {
    if (typeof value === 'string') {
        ...
        if (value[0] !== '#' &&
            ...
            return {
                isValid: false,
                error: new Error(`Invalid color value: ${value} this helper just does a rough check so if this error is wrong you can update this check!`),
            };
        }
    }
    return {
 ...
 */
export const getMaybeHoverColor = (nonHoveredColor) => {
    if (typeof nonHoveredColor === 'string' && getIsValidSporeColor(nonHoveredColor)) {
        ...
        if (!isValidToken) {
            return nonHoveredColor;
        }
        return maybeHoveredColor;
    }
    return nonHoveredColor;
};
